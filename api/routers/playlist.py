...
def add_to_playlist(data: schemas.PlaylistCreate, db: Session = Depends(get_db)):
...
@router.get("/screen/{screen_key}", response_model=list[schemas.PlaylistOut])
def get_playlist_by_screen(screen_key: str, db: Session = Depends(get_db)):
    ...
    return db.query(models.Playlist).filter_by(screen_id=screen.id).order_by(models.Playlist.order_index).all()
